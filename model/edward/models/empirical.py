...
class Empirical(distribution.Distribution):
    ...
    def __init__(self, params, validate_args=False, allow_nan_stats=True, name="Empirical"):
        with ops.name_scope(name, values=[params]) as ns:
            with ops.control_dependencies([]):
                ...
                super(Empirical, self).__init__(
                    ...
                    parameters={"params": self._params, "n": self._n},
                    is_continuous=False,
    ...
    def _param_shapes(sample_shape):
        return {"params": ops.convert_n_to_tensor(sample_shape, dtype=dtypes.int32)}
    ...
    def _event_shape(self):
        return ops.convert_n_to_tensor(self.get_event_shape)

    def _get_event_shape(self):
        return self._params.get_shape()[1:]

    def _mean(self):
        return tf.reduce_mean(self._params)
    ...
    def sample_n(self, n, seed=None):
        if self.n != 1:
            logits = logit(tf.ones(self.n, dtype=dtypes.float32) /
