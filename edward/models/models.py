...
class PyMC3Model(object):
    ...
    def __init__(self, model):
        ...
        warnings.warn("PyMC3Model is deprecated. Edward is dropping support for mode wrappers"
                      " in future versions; use the native language instead.", DeprecationWarning)
        self.model = model
        self.n_vars = None
    ...
    def log_prob(self, xs, zs):
        ...
        """
        # Store keys so that ``_py_log_prob_args`` knows how each value corresponds to a key.
    ...
    def _py_log_prob_args(self, *args):
        ...
        zs_values = args[:len(self.zs_keys)]

        # Set data placehoders in PyMC3 model (Theano shared variable)
        # to their realizations (Numpy array).
        for key, value in zip(self.xs_keys, xs_values):
            key.set_value(value)
        
        # Calculate model's log density using a dictionary of latent variables.
    ...
    def __init__(self, model=None, *args, **kwargs):
        ...
        **kwargs
        ...
        def _initialize(self):
            ...
            self.n_vars = sum([sum(dim) if sum(dim) != 0 else 1 for dim in self.modelfit.par_dims])
