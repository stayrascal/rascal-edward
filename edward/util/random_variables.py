...
def copy(org_isntance, dict_swap=None, scope="copied", replace_itself=False, copy_q=False):
    ...
    12.0
    """
    if  not isinstance(org_isntance, RandomVariable) and \
        not isinstance(org_isntance, tf.Variable) and \
        not isinstance(org_isntance, tf.Tensor) and \
        not isinstance(org_isntance, tf.Operation):
        raise TypeError("Could not copy instance: " + str(org_isntance))
    
    if dict_swap is None:
        dict_swap = {}
    
    # Swap instance if in dictionary.
    if org_isntance in dict_swap and replace_itself:
        org_isntance = dict_swap[org_isntance]
        if not copy_q:
            return org_isntance
    elif isinstance(org_isntance, tf.Tensor) and replace_itself:
        ...
        for key, value in six.iteritems(dict_swap):
            if isinstance(key, RandomVariable):
                if org_isntance == key.value():
                    if isinstance(value, RandomVariable):
                        org_isntance = value.value()
                    else:
                        org_isntance = value
    ...
    new_name = scope + '/' + org_isntance.name

    # If an instance of the same name exists, return approproately.
    # Do this for random variables
    random_variables = {x.name: x for x in graph.get_collection('_random_variable_collection')}
    if new_name in random_variables:
        return random_variables[new_name]
    
    # Do the same for placeholders. Determine via its op's type.
    if isinstance(org_isntance, tf.Tensor):
        if "Placeholder" in org_isntance.op.type:
            return org_isntance

    if isinstance(org_isntance, RandomVariable):
        rv = org_isntance
        ...
        return new_rv
    elif isinstance(org_isntance, tf.Tensor):
        tensor = org_isntance
        ...
        # Add copied tenfor to collections that the original one is in.
        ...
        # As an isntance of tensorflow.core.framework.graph_pb2.NodeDef, 
        ...
        # It is unique to very Operation type.
        ...
        #Use Graph's private methods to add the top, following
        ...
        if graph._colocation_stack:
            ...
            for colocation_op in graph._colocation_stack:
                ...
                if collocation_op.device:
                    # Make this device match the device of the colocated op, to
                    # provide consistency between the device and the colocation property.
                    if ret.device and ret.device != colocation_op.device:
                        logging.warning("Tried to %s with an op %s that had a different device: %s vs %s. Ignoring colocation property.",
                                        name, colocation_op.name, ret.device, colocation_op.device)
                    else:
        ...
        if (graph._container and 
            ...
            ret.node_def.attr["container"].CopyFrom(attr_value_pb2.AttrValue(s=compat.as_bytes(graph._container)))
